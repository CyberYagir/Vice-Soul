namespace Water2D {
	...
	public class Water2D_Spawner : MonoBehaviour
	{
		...
		IEnumerator loop(Vector3 _pos, Vector2 _initSpeed, int count = -1, float delay = 0f, bool waitBetweenDropSpawn = true){
			...
			//GameObject player = FindObjectOfType<Player2D>().gameObject;
			//CapsuleCollider2D playerCC = player.GetComponent<CapsuleCollider2D>();
			//BoxCollider2D playerBx = player.GetComponent<BoxCollider2D>();
			...
			while (true) {
				for (int i = 0; i < WaterDropsObjects.Length; i++) {
					...
					//for (int p = 0; p < NPCs.Length; p++)
					//{
					//	Physics2D.IgnoreCollision(MetaBall.GetComponent<FluidDamage>().collider2D, NPCs[p].GetComponent<CapsuleCollider2D>(), true);
					//}
					//Physics2D.IgnoreCollision(MetaBall.GetComponent<FluidDamage>().collider2D, playerCC, true);
					//Physics2D.IgnoreCollision(MetaBall.GetComponent<FluidDamage>().collider2D, playerBx, true);
